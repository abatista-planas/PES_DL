...
def generate_true_pes_samples(
    pes_name_list: list[str], n_samples: list[int], size: int, seed: int = 33,device = "cpu"
) -> pd.DataFrame:
    ...
    for i, pes_name in enumerate(pes_name_list):
        if pes_name == "lennard_jones":
            ...
            df = generate_analytical_pes_samples(pes_name, parameters_array, size, device = device , seed =seed)
