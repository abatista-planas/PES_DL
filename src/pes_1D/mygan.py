...
#     for j in range(2, n):
#         count = count + 1
#         loss_train, loss_test,model = get_performance(i,j)
#         print(
#             f"count : {count} -- {i},{j} RMSE Train Loss: {np.sqrt(loss_train)}, Test Loss: {np.sqrt(loss_test)}"
#         )
#         best,worst,mean = check_generator(model, i , j, "lennard_jones", n_samples=1000)
...
# df = pd.DataFrame(
#     {
#         "Train Loss": result[0, 2:, 2:],
#         "Test Loss": result[1, 2:, 2:],
#         "Mean RMSE": result[4, 2:, 2:],
#         "Best RMSE": result[2, 2:, 2:],
#         "Worst RMSE": result[3, 2:, 2:],
#     }
# )

# df.to_pickle('gen_1D_results.pkl')
